...
class EpisodeRecorderWrapper(gym.Wrapper):
    ...
    def step(self, action: WrapperActType) -> tuple[
        ...
        observation=obs
        observation["original_image"]=self.env.render(reduce_res=False)
        if self._is_storing:
            self._steps.append(EpisodeStep(self._prev_obs, int(action)))
            self._prev_obs = deepcopy(observation)
...
def get_episode_gif(path: pathlib.Path) -> pathlib.Path:
    ...
    sh = steps[0].obs["original_image"].shape
    im = Image.new("L", (sh[1], sh[0]), 0)
    images = [
        Image.fromarray(step.obs["original_image"].squeeze() )
        for step in steps
    ]
...
def steps_to_tensors(path: pathlib.Path, total_actions: int) -> \
    ...
    dat = path.read_bytes()
    steps = pickle.loads(dat)
    obs = np.stack([s.obs['screens'] for s in steps])
...
class RewardModelWrapper(gym.Wrapper):
    ...
    def __init__(self, env: gym.Env, model_path: pathlib.Path, dev: torch.device,
        ...
        s = env.observation_space['screens'].shape
    ...
    def step(self, action: WrapperActType) -> tuple[
        ...
        p_obs = np.moveaxis(self._prev_obs['screens'], (2, ), (0, ))
